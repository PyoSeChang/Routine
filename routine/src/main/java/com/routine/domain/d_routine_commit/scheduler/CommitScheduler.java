...
//    }
    @Scheduled( cron = " 0 0 0 * * *")
    public void autoCommitForUpdatingCommitRateAndRewardingPoints() {
        LocalDate targetDate = LocalDate.now().minusDays(1);
        commitService.autoFailUnsubmittedCommits(targetDate);
        commitRateService.updateRateByAutoCommit(targetDate);
        pointService.rewardCircleRoutineCommitToAll(targetDate);
        pointService.rewardCollectiveBonusForEligibleMembers(targetDate);
    }
